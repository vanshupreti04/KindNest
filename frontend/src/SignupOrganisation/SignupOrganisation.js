...
const SignupOrganisation = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const otpResponse = await fetch('http://localhost:5000/api/auth/generate-otp', {
        ...
        body: JSON.stringify({ email: formData.email, userType: 'organisation', userData: formData }),
      });
  ...
  return (
    <div className="signup-organisation-page">
      {step === 'signup' ? (
        <>
          <h1 className="signup-organisation-heading">Signup as Organisation</h1>
          {error && <p className="error-message">{error}</p>}
          <form className="signup-organisation-form" onSubmit={handleSignup}>
            <input
              type="text"
        ...
        <>
          <h1 className="signup-organisation-heading-1">Verify OTP</h1>
          {error && <p className="error-message">{error}</p>}
